...
export const assistantTools = [
  ...
  definition("get_nutrition_intake", "Read own recorded nutrition, target comparison and dietary suggestions. week is trailing seven days; month is trailing thirty days; last_month is the previous calendar month. view=trend describes changes across logged dates. visualization=bar compares recorded protein with its target; visualization=pie shows macro energy proportions. Do not use this for a single planned meal.", schema({ period: periodSchema, view: { type: "string", enum: ["summary", "trend"] }, visualization: { type: "string", enum: ["bar", "pie"] } }, ["period"])),
  ...
  definition("get_health_progress", "Describe recorded weight, health-score or activity trends using calculated differences. week is trailing seven days, month is current calendar month, last_month is previous calendar month. visualization=line returns a time-series chart from the same saved records.", schema({ metric: { type: "string", enum: ["weight", "bmi", "wellness_score", "activity"] }, period: { type: "string", enum: ["week", "month", "last_month"] }, visualization: { type: "string", enum: ["line"] } }, ["metric", "period"])),
...
const properties: Record<string, string[]> = {
  ...
  get_nutrition_intake: ["period", "view", "visualization"], get_recipe: ["date", "mealType", "recipeId", "servings", "view"],
  get_health_progress: ["metric", "period", "visualization"], get_wellness_guidance: ["topic"],
};
...
function invalidArguments(name: string, args: Record<string, unknown>): string | null {
  ...
  if (name === "get_nutrition_intake" && (!["today", "week", "month", "last_month"].includes(args.period as string) || args.view !== undefined && args.view !== "summary" && args.view !== "trend" || args.visualization !== undefined && args.visualization !== "bar" && args.visualization !== "pie")) return "Choose today, week, month or last_month, summary or trend view, and an optional bar or pie visualization.";
  if (name === "get_recipe") {
  ...
  if (name === "get_health_progress" && (!["weight", "bmi", "wellness_score", "activity"].includes(args.metric as string) || !["week", "month", "last_month"].includes(args.period as string) || args.visualization !== undefined && args.visualization !== "line")) return "Choose a supported health metric, week, month or last_month, and an optional line visualization.";
