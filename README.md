...
Online chat reuses the configured DeepSeek provider and the Online AI consent setting. The default model is DeepSeek-V4.1-Flash, requested with the API identifier `deepseek-flash`. Its function calling and JSON responses fit the existing nutrition integration, while keeping one provider simplifies configuration and failure handling. Chat uses temperature 0.2 and top-p 1 with thinking disabled. Concise replies have an output limit of 900 tokens and detailed replies 1600 tokens. Deterministic calculations stay outside the model in both modes.
